...
{
    public partial class Form_Xe_25_Cho : DevComponents.DotNetBar.Office2007RibbonForm
    {
        public Form_Xe_25_Cho(Form_Main fm1)
        {
            InitializeComponent();
            fm = fm1;
        }
                ...
                while (dr.Read() == true)
                {
                    for (int i = 0; i <= grb_25.Controls.Count - 1; i++)
                    {
                        if (dr.GetValue(2).ToString() == grb_25.Controls[i].Text)
                            ((DevComponents.DotNetBar.ButtonX)grb_25.Controls[i]).Image = Properties.Resources.hanh_khach;
                    }
                }
        ...
        #region "Xử lý nhấn chọn chỗ ngồi hoàn tất"
